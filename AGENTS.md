...
| `thts init agents`   | Install agent integration (claude, codex, opencode, gemini, pi, droid) |
...
Droid uses `~/.factory/` globally, native custom droids under `droids/`, and a
standalone `hooks.json`; do not create or modify its user-owned `settings.json`.
...
| `thoughts-locator.md`   | Find documents agent             | Claude, Codex, OpenCode, Droid |
| `thoughts-analyzer.md`  | Analyze documents agent          | Claude, Codex, OpenCode, Droid |
| `thts-session-start.sh` | Hook: bootstrap instructions     | Claude, Gemini, Droid          |
| `thts-prompt-check.sh`  | Hook: keyword detection          | Claude, Gemini, Droid          |
